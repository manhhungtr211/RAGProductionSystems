...
class SemanticCache:
    """
    Redis-backed semantic cache đặt trước bước retrieve.

    Flow:
        query → embed → tìm trong cache (cosine similarity) → HIT: trả về ngay
                                                             → MISS: retrieve bình thường → lưu vào cache
    """
    ...
    def get(self, query: str) -> str | None:
        """
        Tìm trong cache bằng semantic similarity.
        Trả về chuỗi kết quả nếu tìm thấy (score >= threshold), ngược lại None.
        """
        try:
            ...
            print(f"threeshold: {self.threshold}")
    ...
    def set(self, query: str, result: str) -> None:
        """Lưu kết quả retrieve vào cache."""
        try:
    ...
    def invalidate_all(self) -> int:
        """Xóa toàn bộ cache retrieval. Gọi khi S3 / vector store được cập nhật."""
